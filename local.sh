...
E.g.:
./local.sh deploy --target=https://jsonplaceholder.typicode.com --locust-file=https://github.com/ChaithanyaN1109/Locust/blob/main/locustfile.py,https://github.com/ChaithanyaN1109/Locust/blob/main/data.json--slaves=4 --mode=manual
./local.sh deploy --target=https://jsonplaceholder.typicode.com --locust-file=https://github.com/ChaithanyaN1109/Locust/blob/main/locustfile.py./local_path.py --slaves=1 --mode=manual --users=2 --hatch-rate=10 --duration=30

Using positional args:
./local.sh deploy https://jsonplaceholder.typicode.com  ./local_path.py 4 auto 1000 10 300
"
    exit 1
}
...
EOF
    ...
    [ -z "$TARGET" ] && read -p "https://jsonplaceholder.typicode.com" TARGET
    [ -z "$LOCUST_FILE" ] && read -p "https://github.com/ChaithanyaN1109/Locust/blob/main/locustfile.py" LOCUST_FILE
    [ -z "$SLAVES" ] && read -p "1 " SLAVES
    [ -z "$MODE" ] && read -p "manual" MODE
    ...
    if $COMPOSE; then
        ...
        if [ ! -f $COMPOSE_FILE ]; then
            curl -o $COMPOSE_FILE https://github.com/ChaithanyaN1109/Locust/main/docker-compose.yaml
